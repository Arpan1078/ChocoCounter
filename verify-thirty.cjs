...
module.exports=async({evaluate,send,delay,profile})=>{
  ...
  const geometry=async()=>assert(await evaluate(`(()=>{
    ...
    const aligned=[...frame.querySelectorAll('.box-piece')].every((e,i)=>{const b=e.getBoundingClientRect(),p=COUNTER_BOX_LAYOUTS[30].slots[i],img=e.querySelector('img')?.getBoundingClientRect();return Math.abs(b.x+b.width/2-r.x-r.width*p.x/100)<1&&Math.abs(b.y+b.height/2-r.y-r.height*p.y/100)<1&&(!img||(Math.abs(img.x+img.width/2-b.x-b.width/2)<1&&Math.abs(img.y+img.height/2-b.y-b.height/2)<1))});
  ...
  for(const [width,height] of [[1920,1080],[1376,1032],[1180,820],[1024,768]]){
    ...
    await click('[data-capacity-option="30"]');
    await evaluate(`document.querySelector('.box-render').decode()`);
    let previousCount=0;
    for(const count of [0,1,6,12,15,18,24,30]){
      if(count)await qty(count-previousCount);
      previousCount=count;
      assert.equal(await evaluate(`document.querySelector('#foundation-count').textContent`),`${count} / 30`);
      assert.equal(await evaluate(`document.querySelector('.save-box').disabled`),count!==30);
      assert.equal(await evaluate(`document.querySelector('#box-complete').hidden`),count!==30);
      assert.equal((await slots()).length,30);await geometry();
      if([0,15,30].includes(count)){
        const shot=await send('Page.captureScreenshot',{format:'png',captureBeyondViewport:false});fs.writeFileSync(path.join(profile,`thirty-${width}-${count}.png`),Buffer.from(shot.data,'base64'));
      }
    }
    ...
    assert(await evaluate(`document.querySelector('.box-frame').getBoundingClientRect().width>${initial}`));await qty(30);await geometry();
  ...
  await click('[data-capacity-option="30"]');await qty(30);
  ...
  console.log('PASS 30 real asset: 0/1/6/12/15/18/24/30 at four viewports, row-major slots, natural aspect, no scene scroll, resize divider, capacity/duplicates, pointer move/swap/remove, Undo/Reset, Save slot order and unsupported Checkout price; 6/10/16 interactions and 50 real photo verified.');
};
